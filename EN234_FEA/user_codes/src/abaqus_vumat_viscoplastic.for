...
!
       ...
       double precision E,xnu,Y,e0,m,edot0,S,H,td,Omega,alpha
       double precision f,dfde,f1,f2,f3,dfde1,dfde2,dfde3,ddtaddee
       double precision fenmu,fenzi,dfenmu,dfenzi
       double precision eplas,ta,deplas,dta
!
...
!     Elastic increment (either no stress, or no plastic strain rate)
       ...
       do while (err>tol)
         ...
         fenzi=ta*(exp(-deplas/Omega)-1)+Omega*dt/deplas
         fenmu=1+Omega/deplas*exp(-deplas/Omega)
         dta=fenzi/fenmu

         f1=sestar/S-3*E*deplas/(2*(1.d0+xnu)*S)
         f2=Y/S*(1+(eplas+deplas)/e0)**m+log(deplas/(dt*edot0))      
         f3=H*(1-exp(-((ta+dta)/td)**alpha))
         f=f1-(f2+f3)

         dfde1=-3*E*deplas/(2*(1.d0+xnu)*S)
         dfde2=Y/S/e0*m*(1+(eplas+deplas)/e0)**(m-1)+1/deplas    
         
         dfenzi=-ta*exp(-deplas/Omega)/Omega-Omega*dt/deplas**2
         dfenmu=-exp(-deplas/Omega)/(deplas**2)*(Omega+deplas)
         ddtaddee=(dfenzi*fenmu-dfenmu*fenzi)/fenmu**2
         dfde3=H*exp(-((ta+dta)/td)**alpha)*alpha*
     1    ((ta+dta)/td)**(alpha-1)/td*ddtaddee
         dfde=dfde1-dfde2-dfde3
     ...
     2                           *sdevstar(1:ntens)
        stressNew(k,1:ndir) =stressNew(k,1:ndir) + 
     1   (sum(stressOld(k,1:ndir))
     2   +E/(1-2*xnu)*sum(strainInc(k,1:ndir)))/3.d0 ! In HW9 wrong
