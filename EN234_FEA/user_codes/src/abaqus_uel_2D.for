...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !   Variables below are for computing integrals over element faces
      ...
      double precision  ::  xi1(1,6)                            ! 1D integration points
      double precision  ::  w1(6)                              ! Integration weights
      double precision  ::  N1(3)                             ! 1D shape functions
      double precision  ::  dNdxi1(3,1)                         ! 1D shape function derivatives
    ...
    !
      ...
      double precision  ::  dxdxi0(2,2),dxidx0(2,2)           ! Incompatible mode
      double precision  ::  xi0(2)                            ! Incompatible mode
      double precision  ::  detkaa                            ! Incompatible mode
      double precision  ::  Ua(22),ru(18),ra(4)               ! Incompatible mode
    !
    ...
    !     --  Loop over integration points
     ...
     1   + 0.5D0*dot_product(stress,strain)*w(kint)*determinant           ! Store the elastic strain energy
      ...
      elseif (JTYPE == 2) then          ! Incompatible Elements
    ...
    !     --  Calculate at kxi=0 for incompatible mode element
      call abq_UEL_2D_shapefunctions(xi0,NNODE,N,dNdxi)
      dxdxi0 = matmul(coords(1:2,1:NNODE),dNdxi(1:NNODE,1:2))
      call abq_UEL_invert2d(dxdxi0,dxidx0,det0)
    ...
    !     --  Loop over integration points
      do kint = 1, n_points
        ...
        ktemp(1:2*NNODE+4,1:2*NNODE+4)=ktemp+
     ...
     2    ,matmul(D,B(1:4,1:2*NNODE+4)))*w(kint)*determinant
      ...
      Ua(1:2*NNODE)=U(1:2*NNODE)
      Ua((2*NNODE+1):(2*NNODE+4))=alpha
      ...
      do kint = 1, n_points
        ...
        strain = matmul(B(1:4,1:2*NNODE+4),Ua(1:2*NNODE+4))
        stress = matmul(D,strain)
     ...
     1   -matmul(kua(1:2*NNODE,1:4),matmul(kaainv(1:4,1:4),ra(1:4)))
    ...
    !
    !   DELETED BECAUSE HW3 DOES NOT REQUIRE THIS.
